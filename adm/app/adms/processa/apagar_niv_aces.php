...
if (!empty($id)) {
    //Verificar se há usuário cadastro no nível de acesso
    $result_user = "SELECT id FROM adms_usuarios WHERE adms_niveis_acesso_id='$id' LIMIT 1";
    $resultado_user = mysqli_query($conn, $result_user);
    if (($resultado_user) AND ( $result_user->num_row != 0)) {
        $_SESSION['msg'] = "<div class='alert alert-danger'>ERRO: O nível de acesso não pode ser apagado,
    usuários cadastrados neste nível! </div>";
        $url_destino = pg . '/listar/list_niv_aces';
        header("Location: $url_destino");
        
    } else {//Não há nenhum usuário cadastrado nesse nível
        //Pesquisar no BD se há nível com ordem acima do qual será apagado
        ...
        // echo "Apagar Nivel de acesso<br>";
